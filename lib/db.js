...
export async function getSession(phone) {
  try { return await kv.get(userKey(phone, "session")); } catch { return null; }
}

export async function setSession(phone, data) {
  try { await kv.set(userKey(phone, "session"), data, { ex: 3600 }); } catch {} // expires in 1h
}

export async function clearSession(phone) {
  try { await kv.del(userKey(phone, "session")); } catch {}
}
